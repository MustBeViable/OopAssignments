...
public class GroceryListManager {
    ...
    public void addItem(String item, double price, String category, int quantity) {
        item = item.substring(0,1).substring(0,1).toUpperCase() + item.substring(1).toLowerCase();
    ...
    public void displayList() {
        if (!this.groceryList.isEmpty()) {
            ...
            for (Map.Entry<String, Item> grocery : this.groceryList.entrySet()) {
                ...
                System.out.printf("%d. %d pcs  %s        %.2f€ (%.2f€/kpl)  [%s]\n",
                        index, quantity, name, price*quantity, price, category);
                index++;
            }
        } else {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("Total price of the list is: %.2f €\n", listManager.calculateTotalCost());
        ...
        System.out.println("Give the category you want to print out");
        String categoryToBeTested = input.nextLine();




        listManager.displayByCategory(categoryToBeTested);

        System.out.println("\nWhat do you want to update: ");

        String itemToBeUpdated = input.nextLine();
        //very simple capitalize method :-DD
        itemToBeUpdated = (itemToBeUpdated.substring(0,1).toUpperCase(Locale.ROOT) +
                itemToBeUpdated.substring(1).toLowerCase());

        System.out.println(itemToBeUpdated);

        System.out.println("\nPlease update the quantity of " + itemToBeUpdated);

        int updatedQuantity = Integer.parseInt(input.nextLine());

        System.out.printf("\nUpdating quantity for %s to %d\n", itemToBeUpdated, updatedQuantity);

        listManager.updateQuantity(itemToBeUpdated, updatedQuantity);
